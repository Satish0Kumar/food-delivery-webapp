...
const PaymentStatusPage = () => {
  ...
  // Poll payment status — up to 10 attempts × 4s = 40 seconds
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Known terminal failure codes — stop polling
        ...
        if (attempts < MAX) {
          ...
          // After 40 seconds, check DB directly by txnId
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">Please wait — this may take up to 30 seconds</p>
          <p className="text-gray-400 text-xs mt-3">Do not close or refresh this page</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <div className="flex space-x-3">
            <button onClick={() => navigate('/menu')} className="flex-1 border border-orange-200 text-orange-500 font-bold py-3 rounded-2xl hover:bg-orange-50 transition-all">
              🍽️ Order More
            </button>
            <button onClick={() => navigate('/')} className="flex-1 bg-orange-500 text-white font-bold py-3 rounded-2xl hover:bg-orange-600 transition-all">
              Go to Home
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 mb-6">
          <p className="text-sm text-gray-500">Don't worry — you can retry or choose Cash on Delivery instead.</p>
        </div>
        <div className="flex flex-col space-y-3">
          ...
          <button onClick={() => navigate('/checkout')} className="bg-orange-500 text-white font-bold py-3 rounded-2xl hover:bg-orange-600 transition-all">
            Try Again
          </button>
          <button onClick={() => navigate('/menu')} className="text-gray-400 text-sm py-2 hover:text-gray-600 transition-all">
            Back to Menu
          </button>
        </div>
      </div>
